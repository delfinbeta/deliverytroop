...
class Hotel extends Seguridad {
	...
	public function insertar($nombre, $zipcode, $ciudad, $direccion, $imagen) {
		...
		$sql = sprintf("INSERT INTO hoteles(nombre, zipcode, ciudad, direccion, imagen, estado, fecha_registro, usuario_registro, fecha_actualizacion, usuario_actualizacion) VALUES('%s', '%s', '%s', '%s', '%s', 1, CURDATE(), '%d', CURDATE(), '%d')", $nombre, $zipcode, $ciudad, $direccion, $imagen, $_SESSION['usuario_id'], $_SESSION['usuario_id']);
	...
	public function actualizar($id, $nombre, $zipcode, $ciudad, $direccion, $imagen) {
		...
		$sql = sprintf("UPDATE hoteles SET nombre='%s', zipcode='%s', ciudad='%s', direccion='%s', imagen='%s', fecha_actualizacion=CURDATE(), usuario_actualizacion='%d' WHERE id='%d'", $nombre, $zipcode, $ciudad, $direccion, $imagen, $_SESSION['usuario_id'], $id);
	...
	public function desactivar($id) {
		...
		$sql = sprintf("UPDATE hoteles SET estado=0, fecha_actualizacion=CURDATE(), usuario_actualizacion='%d' WHERE id='%d'", $_SESSION['usuario_id'], $id);
	...
	public function datos($id) {
		...
		if($query = mysqli_query($this->conexion, $sql)) {
			if($rhotel = mysqli_fetch_assoc($query)) {
				...
				$this->usuario_registro = $rhotel['usuario_registro'];
				$this->fecha_actualizacion = $rhotel['fecha_actualizacion'];
				$this->usuario_actualizacion = $rhotel['usuario_actualizacion'];
				return true;
			} else {
				$this->error = "ID no aroja resultados";
				return false;
