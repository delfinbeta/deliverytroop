...
$(document).ready(function() {
	...
	$("#form_insertar").submit(function(ev) {
		...
		var $campoZipcode = $(this).find('input[name="codigo"]');
		...
		if(enviar) {
			...
			$.ajax({
        url: "../../ajax/admin/add_zipcode.php",    // URL destino
        type: "POST",
	...
	$("#form_modificar").submit(function(ev) {
		...
		if(enviar) {
			...
			$.ajax({
        url: "../../ajax/admin/edit_zipcode.php",    // URL destino
        type: "POST",
        data: formData,               // Datos del Formulario
        dataType: "JSON",
        processData: false,           // Evitamos que JQuery procese los datos, daría error
    ...
    if(confirm('¿Esta seguro que desea Eliminar este Registro?')) {
    	...
    	$.post("../../ajax/admin/delete_zipcode.php", { id: id }, function(data) {
				console.log("Error: " + data.error);
				console.log("Mensaje: " + data.mensaje);
